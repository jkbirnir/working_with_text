...
Then download your PDFs and put them into the folder with your R project. Then use the following code to scrape it:

```{r}
text_1 <- pdf_text("93-95_8-21-YUGOSLAVIA.pdf")
```

For the rest of the tutorial,  we will use the newspaper article that we scraped.
...
You can also remove words that are not stop words but you'd like to remove. For example, if you accidentally scrape advertisements or other useless details, you can remove those words specifically:

```{r}
tidy_paragraphs <-tidy_paragraphs %>%
  filter(!word %in% c("password","logging","Politico"))
```
