...
Note that the new object (t_para) counts words in the entire tokenized corpus we fed to it (tidy_paragraphs) and did NOT treat each article as a separate entity. You can make a very simple wordcloud using the wordcloud2() function.
...
There alrerady exist quite a few existing dictionaries and methods for sentiment analysis that make our work easier. The tidytext package provides access to several ready made sentiment lexicons.
